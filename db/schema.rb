...
ActiveRecord::Schema.define(version: 2020_03_12_185443) do
  ...
  create_table "statuses", id: :bigint, default: -> { "timestamp_id('statuses'::text)" }, force: :cascade do |t|
    ...
    t.index ["id", "account_id"], name: "index_statuses_wakuwaku_local_20191008", order: { id: :desc }, where: "((local OR (uri IS NULL)) AND (deleted_at IS NULL) AND ((visibility = 0) OR (visibility = 1)) AND (reblog_of_id IS NULL) AND ((NOT reply) OR (in_reply_to_account_id = account_id)))"
